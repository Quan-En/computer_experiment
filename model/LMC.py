"""
Implementation of Linear Model of Coregionalization(LMC)
...
class InitialLMC(nn.Module):
    def __init__(self, input, output_list: list, latent_process_num: int = 1, noise_term: bool = True):
        ...
        ## scale for distance: each latent process have one object
        ...
        self.weight_list = nn.ParameterList([
            nn.Parameter(torch.ones((self.obj_dim, ),device=self.device) * 0.1, requires_grad=True)
            for _ in range(latent_process_num)
        ])
...
class Model(InitialLMC, ModelBase.Func):
